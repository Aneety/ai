...
| `banco` | `validacao` | alta | `DB` | `db-onboarding-acesso` declara migration/rollback D1, seed sanitizado Lia Demonstração, contrato de storage `ONBOARDING_ACESSO_DB`, queries tenant-scoped, fixture negativa e validação leve cobrindo convite, primeiro acesso, confirmação, recuperação, hashes, expiração, auditoria e isolamento. | Aguardando scheduler publicar PR/checks e registrar execução Cloudflare D1-backed da migration, rollback e fixture antes de concluir `banco`. | Executar gate remoto D1-backed do ciclo `banco`; depois, se verde, avançar `backend` com API sobre este contrato. |
| `jobs` | `na` | alta | `job` | — | — | Reavaliar somente se houver mudança contratual aprovada nos documentos normativos. |
| `backend` | `triagem` | alta | `backend` | — | Aguardando ciclo `banco` ficar `concluido` com evidência remota D1-backed neste arquivo. | Executar `backend` depois de concluir `banco` com evidência objetiva. |
...
- 2026-06-01 — ciclo `banco` avançado para `validacao`: `db-onboarding-acesso` agora possui contrato D1 versionável com migration, rollback, seed sanitizado, queries tenant-scoped, fixture negativa e validação leve cobrindo convites, primeiro acesso, confirmação, recuperação, lifecycle com desafios em hash, expiração, auditoria e isolamento; falta evidência remota Cloudflare D1-backed para fechar o ciclo.
