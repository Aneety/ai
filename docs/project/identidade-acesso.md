...
| `banco` | `validacao` | alta | `DB` | `db-identidade-acesso` declara migration/rollback D1, seed sanitizado Lia Demonstração, contrato de storage `IDENTIDADE_ACESSO_DB`, queries tenant-scoped, fixture negativa de sessão revogada/cross-tenant/FK e validação leve de hashes, expiração/revogação, permissões e vínculos compostos por tenant. | Aguardando scheduler publicar PR/checks e registrar execução Cloudflare D1-backed da migration, rollback e fixture antes de concluir `banco`. | Publicar a PR pelo scheduler, obter GitHub Actions verdes e executar validação Cloudflare D1-backed para fechar o ciclo `banco`. |
| `jobs` | `na` | alta | `job` | — | — | Reavaliar somente se houver mudança contratual aprovada nos documentos normativos. |
| `backend` | `triagem` | alta | `backend` | — | Aguardando ciclo `banco` ficar `concluido` com evidência Cloudflare D1-backed neste arquivo. | Executar `backend` depois de concluir `banco` com evidência objetiva. |
...
- 2026-06-01 — ciclo `banco` avança para `validacao`: `db-identidade-acesso` agora possui contrato D1 versionável com migration, rollback, seed sanitizado, queries tenant-scoped, fixture negativa com asserções SQL e validação leve cobrindo hash de credenciais, expiração/revogação de sessões, perfis, permissões, FKs compostas e isolamento; falta evidência remota Cloudflare D1-backed para concluir o ciclo.
