...
| `onboarding-acesso` | Ricardo Malnati | alta | `deploy` | `validacao` | [onboarding-acesso](./onboarding-acesso.md) | `worker-onboarding-acesso` agora tem Worker Cloudflare versionável, `wrangler.toml` sem segredos, contrato mínimo `/health`/`/contract`, testes e validação `deploy:validate`. | Aguardando publicação pelo scheduler, PR/checks remotos e `Cloudflare deploy gate` dry-run para concluir `deploy`. |
