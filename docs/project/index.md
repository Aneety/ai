...
| `tenant-white-label` | Ricardo Malnati | alta | `deploy` | `validacao` | [tenant-white-label](./tenant-white-label.md) | Worker deployable `worker-tenant-white-label` preparado com `wrangler.toml`, handler `/health`/`/contract`, testes e validação de deploy para PR/GitHub Actions e Cloudflare dry-run remoto. | Aguardando PR `codex/deploy-tenant-white-label-*`, checks remotos verdes e `Cloudflare deploy gate` dry-run antes de concluir `deploy`. |
