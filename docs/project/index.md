...
| `tenant-white-label` | Ricardo Malnati | alta | `deploy` | `triagem` | [tenant-white-label](./tenant-white-label.md) | Branch `codex/repositorio-tenant-white-label-2026-05-31` cria raiz física e scaffolds mínimos; [PR #19](https://github.com/Aneety/ai/pull/19). | Aguardando PR do ciclo `repositorio` e GitHub Actions verdes antes de iniciar `deploy`. |
| `identidade-acesso` | Ricardo Malnati | alta | `deploy` | `triagem` | [identidade-acesso](./identidade-acesso.md) | Branch `codex/repositorio-identidade-acesso-2026-05-31` cria raiz física e scaffolds mínimos; [PR #22](https://github.com/Aneety/ai/pull/22). | Aguardando PR do ciclo `repositorio` e GitHub Actions verdes antes de iniciar `deploy`. |
| `onboarding-acesso` | Ricardo Malnati | alta | `deploy` | `triagem` | [onboarding-acesso](./onboarding-acesso.md) | Branch `codex/repositorio-onboarding-acesso-2026-05-31` cria raiz física e scaffolds mínimos; evidência temporária até URL do PR. | Aguardando PR do ciclo `repositorio` e GitHub Actions verdes antes de iniciar `deploy`. |
| `pedidos-customizados` | Ricardo Malnati | alta | `deploy` | `triagem` | [pedidos-customizados](./pedidos-customizados.md) | Branch `codex/repositorio-pedidos-customizados-2026-05-31` cria raiz física e scaffolds mínimos; evidência temporária até URL do PR. | Aguardando PR do ciclo `repositorio` e GitHub Actions verdes antes de iniciar `deploy`. |
...
- `gateway-borda` foi a primeira responsabilidade com raiz canônica concluída em `aneety-platform/apps/gateway-borda/`; `tenant-white-label` também possui raiz canônica em `aneety-platform/apps/tenant-white-label/`; `identidade-acesso` possui raiz canônica em `aneety-platform/apps/identidade-acesso/`; `onboarding-acesso` possui raiz canônica em `aneety-platform/apps/onboarding-acesso/`; `pedidos-customizados` passa a possuir raiz canônica em `aneety-platform/apps/pedidos-customizados/` neste ciclo. As demais responsabilidades seguem sem raiz concreta em `aneety-platform/apps/<responsabilidade>/...`.
...
- 2026-05-31 — `identidade-acesso` avança do ciclo `repositorio` para `deploy` em triagem no branch `codex/repositorio-identidade-acesso-2026-05-31`, com raiz física e scaffolds mínimos; [PR #22](https://github.com/Aneety/ai/pull/22).
- 2026-05-31 — `pedidos-customizados` avança do ciclo `repositorio` para `deploy` em triagem no branch `codex/repositorio-pedidos-customizados-2026-05-31`, com raiz física e scaffolds mínimos; evidência temporária até URL do PR.
