...
| `identidade-acesso` | Ricardo Malnati | alta | `deploy` | `validacao` | [identidade-acesso](./identidade-acesso.md) | Worker Cloudflare versionável preparado em `aneety-platform/apps/identidade-acesso/worker-identidade-acesso` com `wrangler.toml` sem segredos, contrato mínimo `/health`/`/contract`, testes e `deploy:validate`; falta PR/checks remotos e `Cloudflare deploy gate` dry-run. | Gate remoto ainda não executado para este diff; não há run Cloudflare nem SHA de PR verde para aceitar `deploy` como concluído. |
