...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `gateway-borda` | Ricardo Malnati | alta | `deploy` | `bloqueado` | [gateway-borda](./gateway-borda.md) | Branch `codex/deploy-gateway-borda-cloudflare-gate` reforça testes de CORS, versão de contrato e service binding ausente para o `worker-gateway` deployable. | Executor sem remoto Git configurado; publicar branch/PR e aguardar GitHub Actions + Cloudflare gate remoto. |
...
- 2026-06-01 — `gateway-borda` registra bloqueio objetivo no ciclo `deploy`: cobertura local leve foi ampliada, mas o checkout da task não possui remoto Git configurado para publicar PR e acionar GitHub Actions/Cloudflare gate.
