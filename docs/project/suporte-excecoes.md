...
- Issue histórica migrada: Issue histórica #24
...
| --- | --- | --- | --- | --- | --- | --- |
| `repositorio` | `validacao` | media | `arquitetura` | Branch `codex/stitch-mvp-design` cria a raiz `aneety-platform/apps/suporte-excecoes/` com `README.md` raiz e diretórios `db-suporte-excecoes`, `worker-suporte-excecoes` e `mfe-suporte-excecoes`. | Aguardando PR, GitHub Actions e merge em `main` antes de marcar `concluido`. | Abrir PR, aguardar checks remotos e registrar SHA/PR neste arquivo antes de avançar para `deploy`. |
