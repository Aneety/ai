...
| --- | --- | --- | --- | --- | --- | --- |
| `repositorio` | `validacao` | alta | `arquitetura` | Branch `codex/stitch-mvp-design` cria a raiz `aneety-platform/apps/pedidos-customizados/` com `README.md` raiz e diretórios `db-pedidos-customizados`, `worker-pedidos-customizados` e `mfe-pedidos-customizados`. | Aguardando PR, GitHub Actions e merge em `main` antes de marcar `concluido`. | Abrir PR, aguardar checks remotos e registrar SHA/PR neste arquivo antes de avançar para `deploy`. |
...
- Diretriz para `microfrontend`: Telas Stitch úteis: detalhe do pedido, solicitação de orçamento, confirmação de envio/recebimento e rastreio. Manter dados decisórios; CAD/3D/STL/OBJ só como evidência/demo, nunca ferramenta do produto.
