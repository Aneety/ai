...
| `backend` | `bloqueado` | alta | `backend` | `worker-gateway/backend-readiness.json`, `npm run backend:validate` e `/contract` agora documentam a URL publicada, versão de contrato, runtime Workers, bindings e caminhos upstream exigidos para os BFFs `worker-*`. | Os BFFs `identidade-acesso`, `tenant-white-label` e `onboarding-acesso` ainda não têm ciclos `backend` concluídos para `/session`, `/branding` e `/invitations`; hoje estão retidos em `banco`/validação remota. | Concluir `backend` dos três BFFs dependentes e então executar gate remoto do `gateway-borda/backend` contra a URL publicada. |
...
- 2026-06-01 — ciclo `backend` avança para bloqueio objetivo com contrato versionável: `backend-readiness.json`, `backend:validate` e `/contract` registram bindings, caminhos upstream e evidências de publicação existentes, mas a conclusão depende dos ciclos `backend` de `identidade-acesso`, `tenant-white-label` e `onboarding-acesso` para expor `/session`, `/branding` e `/invitations` em Workers publicados.
