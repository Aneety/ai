...
| `publicacao` | `bloqueado` | alta | `processo` | `deploy` concluído com dry-run remoto do Worker em `Cloudflare deploy gate`; PR #41 preparou `publication-evidence.example.json`, `publication:validate` e runbook remoto. Tentativa real de publicação em [`Cloudflare deploy gate` deploy #26731716443](https://github.com/Aneety/ai/actions/runs/26731716443) falhou antes do deploy porque `CLOUDFLARE_API_TOKEN` e `CLOUDFLARE_ACCOUNT_ID` chegaram vazios ao workflow. | Segredos Cloudflare de GitHub Actions não estão configurados/disponíveis para o workflow remoto; sem eles não há URL HTTPS real publicada para registrar. | Configurar/disponibilizar `CLOUDFLARE_API_TOKEN` e `CLOUDFLARE_ACCOUNT_ID` em `Aneety/ai`, reexecutar `Cloudflare deploy gate` em modo `deploy`, executar `smoke` contra a URL resultante e registrar URL, runs e SHA para concluir `publicacao`. |
...
- 2026-06-01 — ciclo `publicacao` fica bloqueado por dependência remota objetiva: a branch prepara o contrato versionável de evidência (`publication-evidence.example.json`), o comando `publication:validate` e o runbook de publicação/smoke, mas a URL HTTPS real ainda depende de `Cloudflare deploy gate` em modo `deploy` após PR gate verde e segredos/permissões Cloudflare disponíveis.
- 2026-06-01 — tentativa real de `publicacao` executada no [`Cloudflare deploy gate` deploy #26731716443](https://github.com/Aneety/ai/actions/runs/26731716443) falhou antes do deploy porque `CLOUDFLARE_API_TOKEN` e `CLOUDFLARE_ACCOUNT_ID` chegaram vazios ao GitHub Actions; o ciclo segue bloqueado até os segredos Cloudflare ficarem disponíveis e o deploy/smoke remoto publicar uma URL HTTPS real.
