...
| `publicacao` | `bloqueado` | alta | `processo` | `deploy` concluído com dry-run remoto do Worker em `Cloudflare deploy gate`; esta branch adiciona `publication-evidence.example.json`, `publication:validate` e runbook remoto para registrar URL HTTPS real, runs `deploy`/`smoke` e SHA sem segredos. | A URL publicada real ainda não existe no repositório e só pode ser obtida por `Cloudflare deploy gate` remoto em modo `deploy` após PR gate verde e segredos/permissões Cloudflare disponíveis. | Publicar remotamente via GitHub Actions `Cloudflare deploy gate` em modo `deploy`, executar `smoke` contra a URL resultante e registrar URL, runs e SHA para concluir `publicacao`. |
