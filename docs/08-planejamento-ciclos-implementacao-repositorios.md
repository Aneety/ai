...
| --- | --- | --- | --- |
| `gateway-borda` | `publicacao` | `tenant-white-label/deploy`, `identidade-acesso/deploy`, `onboarding-acesso/deploy` | Se qualquer dependência ainda não estiver em `deploy=concluido`, o scheduler deve preemptar `gateway-borda/publicacao`, abrir ou avançar a task do primeiro dependente elegível pela ordem canônica de `docs/project/index.md` e só reexecutar o gate remoto do gateway quando os três deploys estiverem verdes. |
