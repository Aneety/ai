...
const placeholderHostPattern = /^aneety-worker-identidade-acesso\.placeholder\.workers\.dev$/i;
...
assert.equal(evidence.responsibility, 'identidade-acesso', 'Publication evidence must target identidade-acesso.');
assert.equal(evidence.cycle, 'publicacao', 'Publication evidence must target the publicacao cycle.');
assert.equal(evidence.modulePath, 'aneety-platform/apps/identidade-acesso/worker-identidade-acesso', 'Publication evidence must target the canonical Worker module path.');
