...
1. Confirmar que `identidade-acesso/deploy` segue `concluido` no painel operacional, com `Cloudflare deploy gate` em modo `dry-run` já registrado.
2. Acionar `Cloudflare deploy gate` em modo `deploy` com `module_path=aneety-platform/apps/identidade-acesso/worker-identidade-acesso`.
...
6. Atualizar `docs/project/identidade-acesso.md` e `docs/project/index.md` com URL real, runs de GitHub Actions/Cloudflare e SHA final antes de marcar `publicacao` como `concluido`.
