...
function request(path, init = {}) {
  return new Request(`https://worker-onboarding-acesso.aneety.example${path}`, init);
}
...
test('health route is deployable without exposing technical details', async () => {
  ...
  assert.equal(body.service, 'worker-onboarding-acesso');
  assert.equal(body.responsibility, 'onboarding-acesso');
...
test('contract route exposes deploy contract when version is valid', async () => {
  ...
  assert.deepEqual(body.dataBoundaries, [
    'onboarding_invites',
    'first_access_tokens',
    'contact_confirmations',
    'recovery_tokens',
    'access_lifecycle_audit',
  ]);
...
test('unsupported contract versions are rejected with a public error', async () => {
  const response = await handleRequest(
    request('/contract', {
      headers: { [CONTRACT_VERSION_HEADER]: '2026-05-31.onboarding-acesso.v0' },
    }),
  );
  const body = await response.json();
